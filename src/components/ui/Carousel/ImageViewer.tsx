...
const ImageViewer = ({ activeItem, handleIndex, src }: Props) => {
  return (
    <div
      className="absolute bottom-0 py-2 w-full mx-auto flex justify-start items-center drop-shadow-2xl ease-in-out transition-all delay-150 bg-gradient-to-t from-black rounded-b-2xl"
      key={src[activeItem]}
    >
      {src?.map((value, index) => {
        let className =
